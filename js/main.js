...
export let playerStats = { 
    ...
    xpMultiplier: 1.0, 
    upgrades: { xp_boost: 0 }, 
    activeBuffs: [], // NOVO: Para buffs temporários (ex: Kaioken)
    activeTransformations: [], // NOVO: Para transformações permanentes/longas (ex: Super Saiyajin, Fusão)
    basePower: 100, // NOVO: Para guardar o poder base antes de buffs/transformações
    baseDefense: 10, // NOVO: Para guardar a defesa base
    baseMaxHealth: 100, // NOVO: Para guardar o HP máximo base
    baseMaxKi: 100, // NOVO: Para guardar o Ki máximo base
};
...
// Elementos da UI (variáveis globais para acesso rápido)
...
export function logMessage(message, className = 'text-gray-300') { 
    ...
    while (logElement.children.length > 50) { // Mantém apenas as últimas 50 mensagens
        logElement.removeChild(logElement.lastChild);
    }
}

/**
 * Carrega o HTML de uma "view" (tela) e injeta no main-content-area.
 * Em seguida, chama a função de inicialização/renderização específica daquela view.
 * @param {string} viewName O nome do arquivo HTML da view (ex: 'status', 'meditation', 'combat-selection', 'character-creation').
 */
export async function loadView(viewName) {
    if (combatState.isActive && viewName !== 'arena') {
        logMessage("Você não pode sair do combate enquanto ele estiver ativo!", 'text-red-500');
        return;
    }
    console.log(`[main.js] Tentando carregar view: ${viewName}`); // DEBUG
    try {
        const response = await fetch(`views/${viewName}.html`);
        if (!response.ok) {
            throw new Error(`Não foi possível carregar a view: ${viewName}.html (Status: ${response.status})`);
        }
        const html = await response.text();
        mainContentArea.innerHTML = html;
        logMessage(`📺 Carregou tela: ${viewName.charAt(0).toUpperCase() + viewName.slice(1)}`, 'text-gray-500');

        // Chama a função de inicialização/renderização da view carregada
        switch (viewName) {
            case 'character-creation':
                ...
                break;
            case 'status':
                updateUI(); // A tela de status só precisa de uma atualização da UI
                break;
        ...
        mainContentArea.innerHTML = `<p class="text-red-500 text-center mt-8">Erro ao carregar a tela. Verifique o console para mais detalhes.</p>`;
    }
}
...
export function loadPlayerState() {
    ...
    if (localData) {
        try {
            ...
            // Mescla os dados salvos com os padrões, garantindo que novos campos não sejam perdidos
            playerStats = { 
                ...playerStats, 
                ...parsedData,
                attributes: { ...playerStats.attributes, ...parsedData.attributes },
                upgrades: { ...parsedData.upgrades, ...playerStats.upgrades }, // Garante que novos upgrades sejam inicializados
                // Garante que novos campos de buffs/transformações sejam inicializados se não existirem no save
            ...
            localStorage.removeItem('rpgPlayerStats'); // Limpa dados corrompidos
        }
    }
    
    if (!playerStats.race) {
        logMessage("Escolha sua Raça para começar sua jornada!", 'text-yellow-400 font-bold');
        loadView('character-creation'); // Carrega a tela de criação
    } else {
        updateCalculatedStats(); // Calcula todos os stats de combate
        logMessage(`Bem-vindo de volta, ${playerStats.name}! Raça: ${playerStats.race}. (Dados Carregados Localmente)`, 'text-yellow-400');
        playerStats.health = playerStats.maxHealth; 
        playerStats.ki = playerStats.maxKi; 
        updateUI(); // Chama updateUI aqui para preencher a sidebar esquerda também
        loadView('status'); // Mostra a tela de status após carregar o personagem
        disableActions(false);
    }
}
    
/**
 * Salva o estado atual do jogador no localStorage.
 */
export function saveLocalState() { 
    try {
        const stateToSave = { 
            name: playerStats.name, race: playerStats.race, level: playerStats.level, xp: playerStats.xp, 
            coins: playerStats.coins, attributePoints: playerStats.attributePoints,
            attributes: playerStats.attributes, learnedTechniques: playerStats.learnedTechniques,
            upgrades: playerStats.upgrades,
            
            // Salva também os stats calculados e de buff para garantir consistência
            power: playerStats.power, maxHealth: playerStats.maxHealth, defense: playerStats.defense,
            maxKi: playerStats.maxKi, health: playerStats.health, ki: playerStats.ki,
...
/**
 * Calcula o multiplicador total de XP com base na raça e upgrades.
 */
export function calculateXpMultiplier(stats) {
    ...
    if (xpUpgradeLevel > 0 && xpUpgradeInfo) multiplier += (xpUpgradeLevel * xpUpgradeInfo.effect);
    return Math.round(multiplier * 100) / 100;
}

/**
 * Recalcula Power, Max Health, Max Ki e Defense com base nos atributos e nível.
 * Esta função agora aplica transformações. Buffs temporários são aplicados em combate.js.
 */
export function updateCalculatedStats() {
    console.log("[main.js] updateCalculatedStats() chamado."); // DEBUG
    ...
    // Calcula os stats BASE (sem buffs/transformações)
    ...
    console.log(`[main.js] Stats BASE Calculados: Power=${playerStats.basePower}, Health=${playerStats.baseMaxHealth}, Defense=${playerStats.baseDefense}, Ki=${playerStats.baseMaxKi}`); // DEBUG

    // Aplica os stats BASE como ponto de partida
    ...
    playerStats.activeTransformations.forEach(transformId => {
        ...
        if (tech && tech.type === 'Transform' && tech.powerMult) {
            playerStats.power = Math.round(playerStats.power * tech.powerMult); // Arredonda para evitar floats
            console.log(`[main.js] Aplicado Transformação Permanente ${tech.name}. Novo Poder: ${playerStats.power}`); // DEBUG
        }
        // Poderíamos ter transformações que afetam HP, KI, Defesa também
    });
...
/**
 * Atualiza os elementos da UI com os stats do jogador.
 * Esta função agora atualiza elementos tanto na sidebar esquerda (index.html)
 * quanto na tela de status (status.html), se carregada.
 */
export function updateUI() { 
    // console.log("[main.js] updateUI() chamado. PlayerStats:", JSON.stringify(playerStats)); // DEBUG
    // --- Atualiza elementos da SIDEBAR ESQUERDA (index.html) ---
    ...
    if (leftPlayerHpVal) leftPlayerHpVal.textContent = `${Math.max(0, Math.floor(playerStats.health))}/${Math.floor(playerStats.maxHealth)}`;
    ...
    if (leftPlayerKiVal) leftPlayerKiVal.textContent = `${Math.max(0, Math.floor(playerStats.ki))}/${Math.floor(playerStats.maxKi)}`;
    ...
    // --- Atualiza elementos da VIEW DE STATUS (status.html), se ela estiver carregada ---
    if (document.getElementById('stats-name-display')) { // Verifica se a view de status está presente
        document.getElementById('stats-name-display').textContent = playerStats.name || '---';
        document.getElementById('stats-race-display').textContent = playerStats.race || '---';
        ...
        document.getElementById('stats-health-display').textContent = `${Math.max(0, Math.floor(playerStats.health))}/${Math.floor(playerStats.maxHealth)}`;
        document.getElementById('stats-health-bar').style.width = `${Math.max(0, healthPercent)}%`;

        const kiPercent = (playerStats.ki / playerStats.maxKi) * 100;
        document.getElementById('stats-ki-display').textContent = `${Math.max(0, Math.floor(playerStats.ki))}/${Math.floor(playerStats.maxKi)}`;
        document.getElementById('stats-ki-bar').style.width = `${Math.max(0, kiPercent)}%`;
        
        document.getElementById('stats-coins-display').textContent = playerStats.coins; 
        document.getElementById('stats-attribute-points-display').textContent = playerStats.attributePoints;
        ...
        if (attrList) {
            attrList.innerHTML = `
                ...
                <li>Vigor (VIT): ${playerStats.vitality}</li> <!-- Correção aqui -->
                <li>Controle de Ki (KI): ${playerStats.ki_control}</li> <!-- Correção aqui -->
            `;
            //DEBUG: verificar se os atributos base estão sendo atualizados corretamente
            console.log(`[main.js] Atributos Base na UI: STR=${playerStats.attributes.strength}, VIT=${playerStats.attributes.vitality}, KI=${playerStats.attributes.ki_control}`);
        }
...
/**
 * Desabilita ou habilita botões de ação e links de menu.
 * NOTA: O botão de ataque da arena será controlado diretamente em combate.js para maior precisão.
 */
export function disableActions(disabled) { 
    // console.log(`[main.js] disableActions(${disabled}) chamado.`); // DEBUG
    document.querySelectorAll('.action-btn, #create-character-button, .menu-link').forEach(element => {
        if (element.tagName === 'A' || element.classList.contains('menu-link')) { // Links do menu
            if (disabled) {
                element.style.pointerEvents = 'none'; // Desabilita clique no link
                element.classList.add('opacity-50', 'cursor-not-allowed');
            } else {
                element.style.pointerEvents = 'auto';
                element.classList.remove('opacity-50', 'cursor-not-allowed');
            }
        } else { // Botões com a classe 'action-btn' ou #create-character-button
...
/**
 * Obtém uma técnica pelo seu ID a partir dos dados da raça do jogador.
 */
...
window.getTechById = getTechById; // Expor globalmente para uso em onclick="" se necessário
...
window.onload = initGame; // Inicia o jogo quando a página carrega

// Expõe loadView globalmente para o HTML
window.loadView = loadView;
// Também expõe disableActions
