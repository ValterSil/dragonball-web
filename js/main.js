...
export let playerStats = { 
    ...
    xpMultiplier: 1.0, 
    upgrades: { xp_boost: 0 }, 
    activeBuffs: [], // Para buffs temporários (ex: Kaioken)
    activeTransformations: [], // Para transformações permanentes/longas (ex: Super Saiyajin, Fusão)
    basePower: 100, // Para guardar o poder base antes de buffs/transformações
    baseDefense: 10, // Para guardar a defesa base
    baseMaxHealth: 100, // Para guardar o HP máximo base
    baseMaxKi: 100, // Para guardar o Ki máximo base
};
...
// Dados das Raças (apenas um exemplo resumido aqui)
export const RACE_DATA = { 
    Saiyajin: { emoji: '🐵', bonus: 'XP +5% / Poder +10% (Base)', xpBonus: 0.05, initialPower: 110, initialDefense: 15, techniques: [] },
    Humano: { emoji: '🧑', bonus: 'Nenhum', xpBonus: 0.0, initialPower: 100, initialDefense: 10, techniques: [] },
    // mais raças aqui...
};

// Dados dos Upgrades (exemplo resumido)
...
// Carrega a view dinâmica pelo nome
export async function loadView(viewName) {
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Não foi possível carregar a view ${viewName}.html`);
...
// Carrega estado do jogador do localStorage
...
// Calcula o multiplicador de XP considerando upgrades e raça
...
// Recalcula stats base e aplica transformações permanentes
...
// Atualiza UI da sidebar e tela status (quando carregada)
...
// Função para desabilitar ou habilitar botões e links do menu
...
// Retorna uma técnica pelo ID a partir dos dados da raça
