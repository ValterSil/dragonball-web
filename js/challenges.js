...
import { doc, collection, getDocs, query, where, updateDoc, onSnapshot, arrayUnion } from "https://www.gstatic.com/firebasejs/9.22.1/firebase-firestore.js";
...
function listenForInvites() {
    ...
    onSnapshot(inviteRef, (snapshot) => {
        ...
        const invite = data.invites[0]; // pega o primeiro convite
        if (invite.from !== currentUserId) { 
            const accept = confirm(`Você recebeu um convite de duelo! Aceitar?`);
            if (accept) {
                startPvpMatch(currentUserId, invite.from);
            }
            // remove o convite aceito/rejeitado
            updateDoc(inviteRef, { invites: [] });
        }
    });
}

async function startPvpMatch(player1Id, player2Id) {
    // Cria partida no Firestore
    const matchRef = doc(collection(db, "pvpMatches"));
    await setDoc(matchRef, {
        player1: { uid: player1Id, stats: {}, lastActionAt: new Date() },
        player2: { uid: player2Id, stats: {}, lastActionAt: new Date() },
        turn: player1Id,
        status: "active",
        createdAt: new Date(),
        updatedAt: new Date()
